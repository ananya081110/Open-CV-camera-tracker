...
def main():
    ...
    # LIVE ENTERPRISE API
    # --------------------------------------------------------
    # Runs in the same process as the AI loop, so the React UI
    # receives the actual YOLO/tracker/retail state and frame.
    try:
        ...
        api_thread.start()
        print("[INFO] Live FastAPI server: http://127.0.0.1:8000")
    except Exception as exc:
    ...
    try:
        ...
        while True:
            ...
            # Publish the exact annotated frame + AI state to React/FastAPI.
            try:
                LIVE_STATE.publish(
                    frame=frame,
                ...
                print(f"[WARNING] Live state publish failed: {exc}")
